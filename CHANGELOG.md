...
- **BREAKING**: Changed `n_samples` and `n_features` from `int` to `long` in Dataset class to support larger datasets
- Updated `getNSamples()` and `getNFeatures()` return types to `long`
- Updated `PartialResult::setSamples()` and `PartialResult::setFeatures()` parameter types to `long`
